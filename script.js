...
function setupEventListeners() {
    ...
    // Inbox Button
    elements.inboxBtn.addEventListener('click', () => showModal('inbox'));
...
function checkAuth() {
    if (state.currentUser) {
        ...
        elements.inboxBtn.style.display = 'none';
        elements.adminBtn.style.display = 'none';
    }
}
...
function createItemCard(item) {
    ...
    if (item.image) {
        ...
        imageHtml = '<div class="item-image empty"></div>';
    }
    ...
    const modalContent = `
        <div class="modal" id="itemDetailModal">
            <div class="modal-content" style="max-width: 700px; padding: 2rem;">
                ...
                ${item.image ? `<img src="${item.image}" style="width: 100%; max-height: 300px; object-fit: cover; border-radius: var(--radius); margin-bottom: 1.5rem;">` : ''}
...
function adminLikeItem(itemId) {
    ...
    showNotification('Admin like added!', 'success');
